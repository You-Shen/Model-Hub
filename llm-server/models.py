...
class User(db.Model, UserMixin):
    ...
    def getAllMessages(self):
        return json.loads(self.current_tongyi_messages), json.loads(self.current_wenxin_messages), json.loads(
            self.current_chatgpt_messages)
...
class UserSettings(db.Model):
    id = db.Column(db.Integer, primary_key=True)
    user_id = db.Column(db.Integer, db.ForeignKey('user.id'), nullable=False)
    prompt = db.Column(db.String(500), default="你是一个助手...")
    temperature = db.Column(db.Float, default=0.7)
    model_priority = db.Column(db.String(100), default="gpt,wenxin,tongyi")  # 用逗号分隔表示数组
